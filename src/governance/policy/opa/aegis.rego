...
# Main Decision Logic
#
# Rules are evaluated as priority-ordered candidates; the highest-priority
# matching rule wins. This avoids eval_conflict_error when multiple rules
# match a single input (deny-oriented rules outrank permissive ones).
# ==============================================================================

# System admin always allowed
candidate contains entry if {
...
# Highest-priority matching candidate wins; default deny when none match
decision := result if {
	some e in candidate
	e.priority == max([p | some c in candidate; p := c.priority])
	result := e.result
}
